(function () {
  ...
  function renderPlans(plans, list) {
    ...
    plans.forEach(function (plan) {
      ...
      var article = document.createElement('article'); article.className = 'signup-plan-card relative flex flex-col rounded-xl border ' + (code === 'FREE' ? 'border-emerald-200' : code === 'PRO' ? 'border-violet-200' : 'border-blue-300') + ' bg-white p-6 shadow-soft'; article.tabIndex = 0; article.setAttribute('role', 'button'); article.setAttribute('aria-label', 'Escolher plano ' + (plan.name || code)); article.dataset.planCode = code; article.dataset.planName = plan.name || code;
      ...
      var button = article.querySelector('button'); article.dataset.planPrice = priceNodes[0].textContent + ' ' + (price.currency || 'MZN'); article.dataset.planCycles = JSON.stringify(plan.billing_cycles || ['monthly']); button.dataset.planCode = code; list.appendChild(article);
    });
  }
